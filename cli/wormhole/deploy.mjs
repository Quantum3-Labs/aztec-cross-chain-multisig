...
import { PXE_URL } from "./constants.js";
...
async function main() {
  ...
  const [ownerWallet, receiverWallet] = await getInitialTestAccountsWallets(
    pxe
  );
  ...
  const wormhole = await Contract.deploy(
    ownerWallet,
    WormholeJsonContractArtifact,
    wormhole_init_params,
    "init"
  )
    .send({
  ...
  await pxe.registerContract({
    instance: wormhole.instance,
  });

  console.log(`Wormhole deployed at ${wormhole.address.toString()}`);

  const addresses = {
    wormhole: wormhole.address.toString(),
  ...
  console.log(
    `Calling publish_message with message "${message}" on wormhole contract...`
  );
  console.log(`Payload: ${payload}`);

  console.log(`Minting tokens to public...`);
  await mintTokensToPublic(token, ownerWallet, ownerAddress, 10000n);
  ...
  if (publish_private) {
    ...
    await mintTokensToPrivate(token, ownerWallet, ownerAddress, 10000n);
    ...
    console.log(
      `${ownerAddress} is transferring ${msg_fee} tokens to ${receiverWallet.getAddress()} in private`
    );

    console.log(
      `Generated private authwit`,
      contract.address,
      ownerWallet.getAddress()
    );

    const _tx = await contract.methods
      .publish_message_in_private(100, payload, msg_fee, 2, ownerAddress, nonce)
    ...
    // const _tx = await contract.methods
    //   .publish_message_in_public(100, payload, msg_fee, 2, ownerAddress, nonce)
    //   .send()
    //   .wait();
    // console.log(_tx);
  }
  ...
  const sampleLogFilter = {
    ...
    toBlock: 190,
    contractAddress:
      "0x081a143b80470311c64f8fd1b67a074e2aa312bf5e22e6ebe0b17c5b3b44470b",
  };
